...
@Alternative
@Stateful
public class EJBUserManager implements UserManager, Serializable{
   ...
   public User findUser(String username, String password) throws Exception {
      ...
      if (results.isEmpty()) {
    	  logger.info("non trovo credenziali che matchano" + username); 
         return null;
      } else if (results.size() > 1) {
         throw new IllegalStateException("Cannot have more than one user with the same username!");
      } else {
    	  logger.info("trovate credenziali che matchano  usr: " + username +" pwd: "+password); 
